...
import {
  startButton, handleButtonJoin, handleButtonStart, handleButtonPress,
} from "./games/button.js";
...
function menuMsg() {
  return (
    ...
    `⚡ <b>الزر الأخير</b>\n<i>آخر واحد يضغط الزر يخسر — وأحياناً البوت يخدعكم 😈</i>`
  );
}
...
function menuKeyboard(chatId: number) {
  return Markup.inlineKeyboard([
    ...
    [Markup.button.callback("⚡  الزر الأخير",             `menu:btn:${chatId}`)],
  ]);
}
...
export async function launchBot(): Promise<void> {
  ...
  bot.command(["button", "الزر", "زر"], (ctx) => {
    if (ctx.chat.type === "private") { ctx.reply("🚫 للقروبات فقط!").catch(() => {}); return; }
    const f = ctx.from;
    startButton(bot, ctx.chat.id, f.id, f.username, f.first_name ?? "", f.last_name ?? "");
  });
  ...
  bot.on("callback_query", async (ctx) => {
    ...
    try {
      ...
      if (data.startsWith("menu:btn:")) {
        const chatId = parseInt(data.slice("menu:btn:".length), 10);
        if (isNaN(chatId)) return;
        if (gameStates.has(chatId)) { await ctx.answerCbQuery("⚠️ في لعبة شغالة!").catch(() => {}); return; }
        await ctx.answerCbQuery("⚡ الزر الأخير!").catch(() => {});
        ctx.editMessageReplyMarkup({ inline_keyboard: [] }).catch(() => {});
        const fr = ctx.from;
        startButton(bot, chatId, fr.id, fr.username, fr.first_name ?? "", fr.last_name ?? "");
        return;
      }
      ...
      // ── الزر الأخير ────────────────────────────────────────────────────────────
      if (data.startsWith("btn:join:")) {
        const chatId = parseInt(data.slice("btn:join:".length), 10);
        if (!isNaN(chatId)) { await handleButtonJoin(bot, ctx, chatId); return; }
      }
      if (data.startsWith("btn:start:")) {
        const chatId = parseInt(data.slice("btn:start:".length), 10);
        if (!isNaN(chatId)) { await handleButtonStart(bot, ctx, chatId); return; }
      }
      if (data.startsWith("btn:press:")) {
        const parts  = data.split(":");
        const chatId = parseInt(parts[2], 10);
        const seq    = parseInt(parts[3], 10);
        if (!isNaN(chatId) && !isNaN(seq)) { await handleButtonPress(bot, ctx, chatId, seq); return; }
      }
